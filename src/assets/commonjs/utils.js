...
export const AppPay = new class {
	constructor(){
        ...
        this.ALIPAYSERVER = store.state.API+'/addons/litestore/api.order/alipay_pay?order_no=';//支付宝支付
		this.WXPAYSERVER = store.state.API +'/addons/litestore/api.order/wechat_pay?order_no=';//微信支付
    ...
    pay(id,order_no,callback){  
        ...
        if(id=='alipay'){  
            ...
            PAYSERVER = this.ALIPAYSERVER+order_no;  
        }else if(id=='wxpay'){  
			that.channel = that.channels[1];
            PAYSERVER = this.WXPAYSERVER+order_no;  
        }else{  
